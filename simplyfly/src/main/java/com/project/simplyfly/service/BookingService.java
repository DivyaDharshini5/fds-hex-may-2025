...
public BookingService(CustomerRepository customerRepository, PassengerRepository passengerRepository,
		BookingRepository bookingRepository, CouponRepository couponRepository,
...
/***1.get the logged in user with principal
 * 2.find seat-selected passenger for the particular customer
 * 3.from passenger get the flight
 * 4.Apply coupon value as discount 
 * save booking and payment in db
 * **/
	...
	//for each passenger make booking
	 ...
	 for(Integer passengerId:bookingPaymentDto.getPassengerIds()) {
		 ...
		 if(passenger.getSeat()==null)
			 throw new ResourceNotFoundException("Passenger ID does not have assigned seats");
		 //get flight through seat 
		 Flight flight=passenger.getSeat().getFlight();
		 if(flight==null)
